#!/usr/bin/env python3
"""
Oriented Bounding Box Detection RTSP Stream Server (com.aiot.OBBTflite)

NPU-accelerated variant of com.aiot.OBB. Runs YOLO11-OBB via TFLite on the
Verisilicon VX delegate (NPU) with automatic CPU fallback. Draws rotated boxes
with class labels and streams H264 over a TCP socket. No tracking.
...
APP_CONFIG_PATH keys:
    ...
    det_nms_iou     (number)  rotated NMS IoU threshold [0.45]
...
Model binding:
    tflite_model    (object)  {"path": "backend/yolo11n-obb_npu.tflite"}
...
# DOTA v1 classes (YOLO11-OBB default training set)
DOTA_CLASSES = [
...
# ── TFLite OBB detector (NPU first, CPU fallback) ─────────────────────────────

class TFLiteDetector:
    """YOLO11-OBB head on TFLite. Returns (obbs_xywhr, scores, classes)."""
    ...
    def infer(self, frame):
        ...
        return pp.decode_obb(pred, self.conf, self.iou, self.input_size, lbp, w, h)
...
def annotate_obb(img, dets):
    """Draw oriented (rotated) bounding boxes with class labels.
    dets is (obbs_xywhr, scores, classes)."""
    if dets is None:
        return
    obbs, scores, classes = dets
    font = cv2.FONT_HERSHEY_SIMPLEX
    for i in range(len(obbs)):
        cls_id = int(classes[i])
        conf = float(scores[i])
        color = _COLORS[cls_id % len(_COLORS)]
        cx, cy, bw, bh, ang = obbs[i]
        rect = ((float(cx), float(cy)), (float(bw), float(bh)),
        ...
        name = DOTA_CLASSES[cls_id] if cls_id < len(DOTA_CLASSES) else str(cls_id)
        label = f"{name} {conf:.0%}"
        x, y = pts[0]
        (tw, th), _ = cv2.getTextSize(label, font, 0.45, 1)
        pad_x, pad_y = 6, 4
        lx2 = x + tw + pad_x * 2
        ly1 = max(0, y - (th + pad_y * 2))
        ly2 = ly1 + th + pad_y * 2
        cv2.rectangle(img, (x, ly1), (lx2, ly2), color, -1, cv2.LINE_AA)
        cv2.putText(img, label, (x + pad_x, ly2 - pad_y), font, 0.45, _LABEL_FG, 1, cv2.LINE_AA)
        ...
        while self.running:
            ...
            if self.detector is not None:
                try:
                    ...
                    print(f"[obb] inference error: {exc}", flush=True)
            annotate_obb(frame, last_dets)
            with self._lock:
...
if __name__ == "__main__":
    ...
    if model_path is None:
        print("[main] no tflite_model configured — streaming without OBB", flush=True)
    elif not model_path.is_file():
        print(f"[main] tflite model not found at {model_path} — streaming without OBB", flush=True)
    else:
        try:
            ...
            print(f"[main] detector init failed: {exc} — streaming without OBB", flush=True)

    server = OBBServer(cfg, detector)
