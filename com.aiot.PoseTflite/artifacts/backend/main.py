#!/usr/bin/env python3
"""
Pose Estimation + Tracking RTSP Stream Server (com.aiot.PoseTflite)

NPU-accelerated variant of com.aiot.Pose. Runs YOLO11 pose via TFLite on the
Verisilicon VX delegate (NPU) with automatic CPU fallback. Track IDs come from
Ultralytics' BYTETracker fed with the raw TFLite detections. Draws skeleton +
keypoints + boxes and streams H264 over a TCP socket.
...
Model binding:
    tflite_model    (object)  {"path": "backend/yolo11n-pose_npu.tflite"}
...
# ── TFLite pose detector (NPU first, CPU fallback) ────────────────────────────

class TFLiteDetector:
    """YOLO11 pose head on TFLite. Returns (boxes_xyxy, scores, keypoints)."""
    ...
    def infer(self, frame):
        ...
        return pp.decode_pose(pred, self.conf, self.iou, self.input_size, lbp, w, h)
    ...
    def update(self, boxes, scores, frame):
        ...
        try:
            ...
            dets = SimpleNamespace(
                ...
                cls=np.zeros(len(boxes), np.float32),
            )
...
def annotate_pose(img, dets, track_ids=None):
    """Draw boxes, skeleton, keypoints and (optional) track IDs.
    dets is (boxes_xyxy, scores, keypoints[N,17,3])."""
    if dets is None:
        return
    boxes, scores, kpts_all = dets
    ...
    for kpts in kpts_all:
        for limb_idx, (start, end) in enumerate(_SKELETON):
            if limb_idx >= len(_LIMB_COLORS):
                break
            if kpts[start, 2] < 0.5 or kpts[end, 2] < 0.5:
                continue
            x_s, y_s = int(kpts[start, 0]), int(kpts[start, 1])
            x_e, y_e = int(kpts[end, 0]), int(kpts[end, 1])
            cv2.line(img, (x_s, y_s), (x_e, y_e), _LIMB_COLORS[limb_idx], 2, cv2.LINE_AA)
        for kpt_idx in range(len(kpts)):
            if kpts[kpt_idx, 2] < 0.5:
                continue
            x, y = int(kpts[kpt_idx, 0]), int(kpts[kpt_idx, 1])
            cv2.circle(img, (x, y), 4, _KPT_COLORS[kpt_idx % len(_KPT_COLORS)], -1, cv2.LINE_AA)
    ...
    def _inference_loop(self):
        ...
        while self.running:
            ...
            if self.detector is not None:
                try:
                    boxes, scores, kpts = self.detector.infer(frame)
                    ids = None
                    if self.tracker is not None:
                        ids = self.tracker.update(boxes, scores, frame)
                    last_dets = (boxes, scores, kpts)
                    last_ids = ids
                except Exception as exc:
                    print(f"[pose] inference error: {exc}", flush=True)
            annotate_pose(frame, last_dets, last_ids)
            with self._lock:
...
if __name__ == "__main__":
    ...
    if model_path is None:
        print("[main] no tflite_model configured — streaming without pose", flush=True)
    elif not model_path.is_file():
        print(f"[main] tflite model not found at {model_path} — streaming without pose", flush=True)
    else:
        try:
            ...
            print(f"[main] detector init failed: {exc} — streaming without pose", flush=True)

    server = PoseServer(cfg, detector)
