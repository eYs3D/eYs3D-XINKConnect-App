#!/usr/bin/env python3
"""
Semantic Segmentation RTSP Stream Server (com.aiot.SemanticTflite)

NPU-accelerated variant of com.aiot.Semantic. Runs YOLO11-seg via TFLite on the
Verisilicon VX delegate (NPU) with automatic CPU fallback. Merges instance masks
by class and overlays them with a legend, then streams H264 over a TCP socket.
...
Model binding:
    tflite_model    (object)  {"path": "backend/yolo11n-seg_npu.tflite"}
...
class TFLiteDetector:
    """YOLO11-seg head on TFLite. Returns (boxes_xyxy, scores, classes, masks)."""
...
def annotate_semantic(img, dets, names, input_size, alpha):
    """Merge instance masks by class, overlay colored masks, draw a legend.
    ...
    for i in range(len(masks)):
        ...
        class_masks[cls_id] = m if cls_id not in class_masks else (class_masks[cls_id] | m)

    overlay = img.copy()
    for cls_id, m in class_masks.items():
    ...
    row_h = 22
    margin = 10
    cls_list = sorted(class_masks.keys())
    if not cls_list:
        return
    max_tw = max(cv2.getTextSize(names.get(c, str(c)), font, font_scale, 1)[0][0]
                 for c in cls_list)
    panel_w = dot_size + pad_x * 3 + max_tw
    panel_h = row_h * len(cls_list) + pad_y * 2
    x0, y0 = margin, margin
    panel = img[y0:y0 + panel_h, x0:x0 + panel_w].copy()
    dark = np.zeros_like(panel)
    cv2.addWeighted(dark, 0.55, panel, 0.45, 0, panel)
    img[y0:y0 + panel_h, x0:x0 + panel_w] = panel
    for row, cls_id in enumerate(cls_list):
        color = _COLORS[cls_id % len(_COLORS)]
        label = names.get(cls_id, str(cls_id))
        cy = y0 + pad_y + row * row_h + row_h // 2
        cv2.circle(img, (x0 + pad_x + dot_size // 2, cy), dot_size // 2, color, -1, cv2.LINE_AA)
        tx = x0 + pad_x + dot_size + pad_x
        _, th = cv2.getTextSize(label, font, font_scale, 1)[0]
        cv2.putText(img, label, (tx, cy + th // 2), font, font_scale, (230, 230, 230), 1, cv2.LINE_AA)
...
class SemanticServer:
    ...
    def _inference_loop(self):
        ...
        while self.running:
            ...
            annotate_semantic(frame, last_dets, self.names, self.input_size, self.mask_alpha)
            with self._lock:
...
if __name__ == "__main__":
    ...
    server = SemanticServer(cfg, detector, names)
