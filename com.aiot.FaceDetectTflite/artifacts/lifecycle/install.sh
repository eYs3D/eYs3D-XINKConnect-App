...
echo "[$(date)] Installing com.aiot.FaceDetectTflite..."
...
echo "[$(date)] Probing Vivante NPU delegate..."
$PYTHON - <<'EOF'
import os, sys
...
try:
    ...
    print("[NPU probe] tflite_runtime not available — NPU path disabled", flush=True)
    sys.exit(0)

if not os.path.exists(delegate_path):
    print(f"[NPU probe] {delegate_path} not found — will use TFLite CPU", flush=True)
    sys.exit(0)

try:
    d = tflite.load_delegate(delegate_path)
    print(f"[NPU probe] OK — Vivante NPU hardware acceleration ENABLED", flush=True)
    print(f"[NPU probe] delegate: {d}", flush=True)
except Exception as e:
    print(f"[NPU probe] delegate load failed: {e} — will use TFLite CPU", flush=True)
EOF

echo "[$(date)] com.aiot.FaceDetectTflite installed successfully."
